...
function playSong(url, title, artist) {
  ...
  if (trackName)   trackName.textContent  = title  || 'Unknown';
  if (trackArtist) trackArtist.textContent = artist || 'Unknown';
}
...
async function loadUploads() {
  ...
  try {
    ...
    if (songsCount) {
      songsCount.textContent = uploads.length + ' tracks';
    }

    uploads.forEach((u, i) => {
      const url = API_URL + u.fileUrl;
      ...
      row.innerHTML = `
        <div>${i + 1}. ${u.title}</div>
        <button onclick="playSong('${url}', '${u.title}', '${u.userId?.username}')">Play</button>
        <button onclick="likeUpload('${u._id}', this)">Like</button>
      `;
    ...
    container.innerHTML = `
      <div style="text-align:center;color:red">
        Could not load songs — server issue
      </div>
    `;
  }
}
...
async function likeUpload(id, btn) {
  try {
    const res = await fetch(`${API_URL}/api/uploads/${id}/like`, {
      method: 'PUT'
    });
...
function updateSubscription() {
  ...
  if (currentLikes >= 10) {
    if (subLabel) subLabel.textContent = 'Premium Active';
    if (subSub) subSub.textContent = 'Enjoy premium features';
    ...
    if (subLabel) subLabel.textContent = 'Premium Subscription';
    if (subSub) {
      subSub.textContent = `Like ${10 - currentLikes} more songs to unlock premium`;
    }
  }
}
